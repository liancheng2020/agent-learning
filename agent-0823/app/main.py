...
def create_app(root: Path = ROOT, approval_database: Path | None = None) -> FastAPI:
    review_agent = build_agent(root, "tuned")
    retriever: KnowledgeRetriever = review_agent.retriever
    approvals = ApprovalStore(approval_database or root / "data" / "approvals.db")
    application = FastAPI(title="Frontend Review Approval Agent", version="3.0.0")
    ...
    def health() -> dict[str, object]:
        return {"status": "ok", "service": "frontend-review-approval-agent", "approval_store": "sqlite"}
