...
本文档整理 `agent-0503` 到 `agent-0829` 的学习路径、项目说明、运行方式和面试讲法。
...
目标：把主作品部署边界、失败体验、对外文档和面试材料一次补齐。

- `agent-0826`: Docker Compose 四服务部署
- `agent-0827`: 五类故障演练与页面反馈
- `agent-0828`: 作品级 README、截图与 GIF
- `agent-0829`: 三类岗位简历描述、10 道面试题和投递看板
...
`agent-0809` 到 `agent-0829` 使用 Python，基本运行方式为：
...
| `agent-0826` | Compose 部署 | 编排 Nginx、FastAPI、PostgreSQL/pgvector 和 Redis |
| `agent-0827` | 故障演练 | 覆盖模型、工具、检索、JSON 与审批失败的可理解反馈 |
| `agent-0828` | 作品文档 | 重写架构、选型、评测和失败处理，生成桌面/移动端截图与 GIF |
| `agent-0829` | 面试与投递 | 准备三类岗位描述、10 道问答和每日精投跟踪表 |

推荐先运行 `agent-0815` 理解服务化 Agent，再运行 `agent-0822` 理解 RAG 与评测闭环，最后用 `agent-0829` 展示完整作品并准备面试。
...
- 接入生产级 embedding 与真实模型容量验证
- 使用标准 MCP SDK
- 支持真实 Git diff 和项目文件读取
- 持续扩充来自真实项目的 eval dataset
- 身份认证、RBAC、密钥托管和审计导出
- OpenTelemetry、并发压测、备份和数据迁移
...
- 第 3 周 `agent-0823` 到 `agent-0829`：完成审批、Trace、Redis、Compose、故障演练、作品文档和求职材料。
- 当前主作品为 `agent-0829`，`agent-0822` 用于展示 RAG 评测闭环，`agent-0815` 用于展示服务化过程。
...
最后用 0823-0829 补齐审批、Trace、缓存、部署、故障演练和求职交付。
```

重点展示：

- `agent-0829`：主作品，展示 RAG、审批、Trace、Redis、部署、故障演练和求职材料
...
当前目标不是“看完 Agent 教程”，而是能拿着 `agent-0829` 讲清楚：
...
- Compose 中前端、API、PostgreSQL/pgvector 和 Redis 的职责边界
- 五类失败如何被观测、阻断、降级并反馈给用户
