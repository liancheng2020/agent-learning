...
def match_rules(diff_text: str, version: str = "tuned") -> list[Rule]:
    added = "\n".join(line[1:] for line in diff_text.splitlines() if line.startswith("+") and not line.startswith("+++"))
    rules = RULES if version == "tuned" else [rule for rule in RULES if rule.category in BASELINE_CATEGORIES]
