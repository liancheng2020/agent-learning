...
def build_agent(root: Path, version: str = "tuned") -> ReviewAgent:
    ...
    if version == "baseline":
        return ReviewAgent(KnowledgeRetriever(store, rerank=False, topic_boost=0), top_k=1, version="baseline")
    return ReviewAgent(KnowledgeRetriever(store, rerank=True, topic_boost=0.2), top_k=3, version="tuned")
